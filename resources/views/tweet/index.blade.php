...
<x-app-layout>
      ...
      <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white dark:bg-gray-800 border-b border-grey-200 dark:border-gray-800">
          <table class="text-center w-full border-collapse">
            ...
            <tbody>
              <!-- ブレードではアットマークで始まるワードをディレクティブ(指示)として認識する -->
              @foreach ($tweets as $tweet)
              <tr class="hover:bg-gray-lighter">
                <td class="py-4 px-6 border-b border-gray-light dark:border-gray-600">
                <!-- ブレードテンプレートでは波括弧2つで囲って処理を指示できる -->
                <!-- tweet.show(show())にtweetのidを送る -->
                <a href="{{ route('tweet.show',$tweet->id) }}">
                <!-- インデックスページに投稿したユーザー名を表示 -->
                <!-- ダラーtweetのuserのnameを取得している -->
                ...
                <div class="flex">
                  <!-- favorite 状態で条件分岐 -->
                    ...
                    <form action="{{ route('favorites',$tweet) }}" method="POST" class="text-left">
                      @csrf
                      <x-primary-button class="ml-3" id="ml-3" onclick="start()">
                    ...
                    <form action="{{ route('tweet.destroy',$tweet->id) }}" method="POST" class="text-left">
                      <!-- アットマークmethodでPOSTのdeleteを記述できる   -->
                      @method('delete')
                      <!-- 以下はセキュリティ面より必要な記述。トークンのやり取りで照合を取る -->
                      @csrf
                      <x-primary-button class="ml-3">
  ...
  <!-- <script>
    $("#ml-3").on("click", function(){
      location.href="resources/views/tweet/create.blade.php";
    });
  </script> -->
